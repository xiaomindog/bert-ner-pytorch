...
from models.losses import FocalLoss, LabelSmoothingCrossEntropy, DiceLoss
...
class BertSpanForNer(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None):
        ...
        sequence_output = outputs[0]
        sequence_output = self.dropout(sequence_output)
        start_logits = self.start_fc(sequence_output)  # batch 87 9
       # print("start_logits:", start_logits.shape)
        if start_positions is not None and self.training:
            if self.soft_label:
                ...
                label_logits = start_positions.unsqueeze(2).float()# batch max_len 1
               # print("label_logits1:", label_logits.shape)
        else:
            label_logits = F.softmax(start_logits, -1)
            if not self.soft_label:
                label_logits = torch.argmax(label_logits, -1).unsqueeze(2).float()
        #print("label_logits2:", label_logits.shape)
        end_logits = self.end_fc(sequence_output, label_logits)
        #print("end_logits2:", end_logits.shape)
